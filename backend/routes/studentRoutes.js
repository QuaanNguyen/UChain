...
router.get("/:stid", async (req, res) => {
  try {
    ...
    const university = school
      ? await University.findOne({ schools: school.sid })
      : null;
    ...
    res.json({
      ...
      university: university
        ? { uid: university.uid, name: university.name }
        : null,
      school: school ? { sid: school.sid, name: school.name } : null,
      department: department
        ? { did: department.did, name: department.name }
        : null,
  ...
  try {
    ...
    for (const hash of student.enrollments) {
      try {
        ...
        if (
          normalizedHash.length !== 66 ||
          !/^0x[0-9a-fA-F]{64}$/.test(normalizedHash)
        ) {
        ...
        let onChainData;
        try {
          onChainData = await contract.methods
            .enrollments(normalizedHash)
            .call();
        } catch (callError) {
          const errorMsg = callError.message || String(callError);
          if (
            errorMsg.includes("decoding") ||
            ...
            verificationResults.push({
              hash: normalizedHash,
              valid: false,
              reason: `Hash not found on blockchain: ${errorMsg.substring(
                0,
                100
              )}`,
            });
        ...
        if (Array.isArray(onChainData)) {
          ...
          throw new Error(
            `Unexpected data format from blockchain: ${typeof onChainData}`
          );
        }

        // Check if hash exists (sid != 0 indicates empty/default values)
        const sidValue =
          typeof oc_sid === "bigint" ? oc_sid.toString() : oc_sid.toString();
        if (sidValue === "0" || oc_sid === 0 || oc_sid === "0") {
          verificationResults.push({
            hash: normalizedHash,
            valid: false,
            reason:
              "Hash not found on blockchain (returns default/empty values)",
          });
          allValid = false;
          continue;
        }
        ...
        const gradeMatch =
          String(mongoEnr.grade || "").trim() === String(oc_grade || "").trim();
        const statusMatch =
          String(mongoEnr.status || "").trim() ===
          String(oc_status || "").trim();
        ...
        verificationResults.push({
          ...
          reason: dataMatches
            ...
            : `Data mismatch: grade ${gradeMatch ? "✓" : "✗"} status ${
                statusMatch ? "✓" : "✗"
              }`,
        });
